...
function AdminPage() {
    ...
    const toggleProductStatus = async (id, currentStatus) => {
        try {
            await axiosClient.put(`/products/${id}`, { status: currentStatus === 1 ? 0 : 1 });
    ...
    const deleteProduct = async (id) => {
        if (!window.confirm('Bạn có chắc muốn xóa/ẩn sản phẩm này?')) return;
        try {
    ...
    const confirmOrder = async (id) => {
        try {
            await axiosClient.put(`/orders/${id}/status`, { status: 1 });
    ...
    const toggleUserStatus = async (id, currentStatus) => {
        try {
            await axiosClient.put(`/users/${id}/status`, { status: currentStatus === 1 ? 0 : 1 });
            setUsers(users.map((u) => (u.id === id ? { ...u, status: currentStatus === 1 ? 0 : 1 } : u)));
                ...
                <aside className="sidebar open">
                    ...
                    <div className="middle-sidebar">
                        <ul className="sidebar-list">
                            <li
                                className={`sidebar-list-item tab-content ${activeSection === 'dashboard' ? 'active' : ''}`}
                            >
                                <button type="button" className="sidebar-link" onClick={() => setActiveSection('dashboard')}>
                            ...
                            <li
                                className={`sidebar-list-item tab-content ${activeSection === 'products' ? 'active' : ''}`}
                            >
                                <button type="button" className="sidebar-link" onClick={() => setActiveSection('products')}>
                                    <div className="sidebar-icon">
                                        <i className="fa-light fa-pot-food"></i>
                                    </div>
                                    <div className="hidden-sidebar">Sản phẩm</div>
                                </button>
                            </li>
                            <li
                                className={`sidebar-list-item tab-content ${activeSection === 'users' ? 'active' : ''}`}
                            >
                                <button type="button" className="sidebar-link" onClick={() => setActiveSection('users')}>
                                    <div className="sidebar-icon">
                            ...
                            <li
                                className={`sidebar-list-item tab-content ${activeSection === 'orders' ? 'active' : ''}`}
                            >
                                <button type="button" className="sidebar-link" onClick={() => setActiveSection('orders')}>
                                    <div className="sidebar-icon">
                    ...
                    <div className={`section product-all ${activeSection === 'products' ? 'active' : ''}`}>
                        <div className="admin-control">
                            <div className="admin-control-left">
                                <span style={{ fontWeight: 600 }}>Danh sách sản phẩm</span>
                            </div>
                                ...
                                <tbody>
                                    {products.map((item) => (
                                        <tr key={item.id}>
                                            <td>{item.id}</td>
                                            <td>
                                                <img src={item.img} alt="" className="prd-img-tbl" />
                                            </td>
                                            <td>{item.title}</td>
                                ...
                                <tbody>
                                    {users.map((u, idx) => (
                                        <tr key={u.id}>
                                            ...
                                            <td>{u.phone}</td>
                                            <td>{u.email || '—'}</td>
                                            <td>{u.userType === 1 ? 'Admin' : 'Khách'}</td>
                                ...
                                <tbody>
                                    {orders.map((order) => (
                                        <tr key={order.id}>
                                            ...
                                            <td>{order.status === 1 ? 'Đã xử lý' : 'Chưa xử lý'}</td>
                                            <td>
                                                {order.status === 0 && (
                                                    <button type="button" className="btn-control-large" onClick={() => confirmOrder(order.id)}>
                                                        Duyệt
                                                    </button>
                                                )}
                                            </td>
